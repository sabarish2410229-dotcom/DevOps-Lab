...
------------------------------
Reads the processed CSV, splits into train/test sets, scales numeric
features with StandardScaler, and writes both to data/features/.
...
def build_features(df: pd.DataFrame, target_column: str, test_size: float, random_state: int):
...
def main():
    fe_params = load_params()["feature_engineering"]

    df = load_processed_data()
    train_df, test_df, scaler = build_features(
        df,
        target_column=fe_params["target_column"],
